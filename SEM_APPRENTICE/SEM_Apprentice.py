...
**SEM_APPRENTICE IS THE PROPERTY OF THE AUTHORS AND OWNERS OF SEM_APPRENTICE (ANUDHA MITTAL) AND MAY NOT BE DISTRIBUTED, COPIED, SOLD, OR USED WITHOUT THE EXPRESS CONSENT FROM THEM.**
...
def on_scroll(x, y, dx, dy):
    ...
    if x<2 and y<2:
        ...
        if not lock:  # if lock is False, set to True and display message to suspend Listener
            ...
            stop_record = messagebox.askyesno('STOP RECORDING?', 'Are you sure you want to stop recording?')
...
root.focus_set() # Set the messagebox as the top window
root.attributes('-topmost', True)  # keep window on top of others
start_record = messagebox.askyesno('START RECORDING?', 'Are you sure you want to start recording?')
root.destroy()  # destroy the root window
